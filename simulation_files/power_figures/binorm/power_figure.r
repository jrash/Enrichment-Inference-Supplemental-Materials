setwd("G:\\My Drive\\JHO_lab\\recall_curve_papers\\JASA\\2021-09-07\\power_figures\\binorm")
source("compare_power.r")
source("convenience.r")

pdf("power_figure.pdf")
pi.0 = 1/500; m = 150000; rlim = .1;
plus2 = F; lift.ylim = 50; mu = c(sqrt(2) * .8, sqrt(2) * .6);
distr = "binorm";
...
F.plus1 <- pnorm(c, mean = mu[1], sd = 1)
F.plus2 <- pnorm(c, mean = mu[2], sd = 1)
F.neg <- pnorm(c, mean = mu[2], sd = 1)
f.neg <- dnorm(c, mean = 0, sd = 1)
f.plus1 <- dnorm(c, mean = mu[1], sd = 1)
f.plus2 <- dnorm(c, mean = mu[2], sd = 1)

# PDFs
col.list <- list(c("blue", "orange"), c("green", "orange"))
params.ls <- list(c(mu[1], 0), c(mu[2], 0)) 
...
load("power_all_binorm_unpooled_nocorr.rdata")
ls <- k.ls
